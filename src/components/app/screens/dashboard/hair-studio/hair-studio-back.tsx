// src/components/app/screens/hair-studio/hair-studio-back.tsx
"use client";
...
interface HairStudioBackProps {
  /** Where the back button navigates to (default: "/app") */
  backHref?: string;
  /** Show the credits badge on the right */
  showCredits?: boolean;
  /**
   * 🆕 Increment this to force a re-fetch of user credits.
   * Pass a changing number from parent after mutations.
   */
  refreshKey?: number;
}
  ...
  return (
    <motion.span
      ...
      className={cn(
        ...
        "bg-linear-to-r from-[#D99A5B]/20 via-[#D99A5B]/40 to-[#D99A5B]/20",
...
export function HairStudioBack({
  backHref = "/app",
  showCredits = true,
  refreshKey = 0,
}: HairStudioBackProps) {
  ...
  // ─── Fetch credits (re-runs on refreshKey change) ───
  React.useEffect(() => {
    let cancelled = false;
    ...
    async function fetchCredits() {
      ...
      try {
        ...
        if (cancelled) return;
        console.error("[HairStudioBack] Fetch error:", err);
        setCredits(0);
      } finally {
    ...
    // 🆕 refreshKey in deps → re-fetches whenever parent bumps it
